# Running the co-occurrence analysis for ANSC516 Project
...
str(metadata)
colnames(metadata)[4] = "randomizationgrp"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$randomizationgrp))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$randomizationgrp)

my_column <- "randomizationgrp"
n1 <- 8
n2 <- 8

num_metadata_columns <- 10
...
for(i in 1:length(treatments)){
  ...
  }   else if (treatments[i]==treatments[8]) {
    merged<-merge(merged,subset(ns_melt, n > n8),by=c("Var1","Var2"))
  }   else
...
weightloss_ASVs<-subset(dataset, get(my_column)==treatments[1])

colnames(weightloss_ASVs[1:10])
head(final_results)
ggplot(weightloss_ASVs, aes(x = ASV1, y = ASV141)) +
  geom_point()
...
write.csv(strong_results_taxa, "output/project-results-taxa.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="weightloss"), "output/project-results-taxa-weightloss.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="meddiet"), "output/project-results-taxa-meddiet.csv", row.names = F)
