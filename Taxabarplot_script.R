...
setwd("C:/Users/Administrator/OneDrive/Desktop/ANSC516_output/Project_OutputR")
...
##Qiime2r method of reading in the metadata files
metadata<-read_q2metadata("metadata_project_updated.txt")
str(metadata)

##Qiime2r method of reading in the table files
rare_table<-read_qza("rarefied_table.qza")
parse_table<-rare_table$data
...
physeq <- qza_to_phyloseq(
  ...
  metadata = "metadata_project_updated.txt"
)
...
levels(metadata$randomizationgrp)
metadata$randomizationgrp.ord = factor(metadata$randomizationgrp, c("meddiet", "weightloss"))
levels(metadata$randomizationgrp.ord)
...
colnames(meta.physeq)[colnames (meta.physeq) =="randomizationgrp"]<-"Treatment"
...
for(ml in my_level){
  ...
  physeq_meta_filtered$Treatment.ord = factor(physeq_meta_filtered$Treatment, c("meddiet", "weightloss"))
  ...
  ggplot(physeq_meta_filtered, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
    ...
    ylim(c(0,1)) +
    guides(fill = guide_legend(reverse = F, keywidth = .8, keyheight = .5, ncol = 1)) +
  ...
  ggsave(paste0("output", ml, "BarPlot_", my_column, ".png"), height = 4.5, width = 5)
}
...
run_deseq2 <- function(my_factor, x, y){
  ...
  DESeq_fig = ggplot(sigtab, aes(x=Phylum, y = log2FoldChange, color=Phylum)) + 
    geom_point(size=2) + 
    ylab(paste0("(", my_contrast[2], "/", my_contrast[3], ")\n", "log2FoldChange")) +
    scale_color_manual(values = my_colors[c(4,6,8,10,12,14,16,18,20)]) +
    #ylim(0,8) +
    geom_text(color="black", x=length(unique(sigtab$Phylum))-1, y=max(sigtab$log2FoldChange)-1, label=my_contrast[2], show_guide = F) +
    geom_text(color="black", x=3, y=min(sigtab$log2FoldChange)+1, label=my_contrast[3], show_guide = F) +
    theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
  
  ggsave(paste0("output", my_contrast[3], "-", my_contrast[3], ".png"), DESeq_fig, height = 4, width = 5)
}


run_deseq2("Treatment", "meddiet", "weightloss")